	...
	<view class="rf-category">
		<view
			...
			<scroll-view scroll-y="true" class="left">
				<view class="row" v-for="(fItem, fIndex) in categoryList" :key="fItem.id" :class="[fIndex === showCategoryIndex ? 'on' : '']" @tap="showCategory(fItem,fIndex)">
					<view class="text">
			...
			<scroll-view scroll-y="true" class="right">
				<view v-for="(fItem, fIndex) in categoryList" :key="fItem.id">
					<view v-if="fIndex === showCategoryIndex" class="category">
						<view v-if="fItem && fItem.modifyName" class="banner" @tap="indexTopToDetailPage(fItem.delName)">
							<image :src="fItem && fItem.modifyName" mode="aspectFill"/>
						</view>
						<view class="rf-drawer-content rf-flex-attr">
							<view class="rf-attr-item" :class="[item1.isActive ? `bg-${themeColor.name} rf-btmItem-active` : 'rf-btmItem-normal']"
										v-for="(item1, index) in  fItem.childCateGory"
						...
						<view class="list" v-if="categoryProductList && categoryProductList.length > 0">
							<view class="box" v-for="tItem in categoryProductList" :key="tItem.id" @tap.stop="navTo(`/pages/product/product?id=${tItem.id}`)">
...
import {
	...
	cartItemCount,
	cartItemCreate,
	queryAllFirstAndSecondCategory
} from '@/api/product';
...
export default {
	...
	methods: {
		...
		async getProductCate(type) {
			await this.$http.get(`${queryAllFirstAndSecondCategory}`).then(async r => {
				if (type === 'refresh') {
		...
		// 获取广告列表
		async getAdvList() {
			await this.$http.get(`${bannerList}`, {parmas:{orderCount: 5}}).then(r => {
				this.cateTop = r[0];
			});
		},
		...
		async getProductList() {
			...
			params.secondCateId = this.currentCateId;
			await this.$http.get(`${productList}`, {params:params}).then(async r => {
