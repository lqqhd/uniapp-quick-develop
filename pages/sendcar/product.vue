<template>
	<view class="product">
<!--		<jxb-sendcar-detail @product="getProductDetail" :userInfo="userInfo" :url="currentUrl" :product="productDetail"></jxb-sendcar-detail>-->
    <!--回到顶部-->
...
import { productDetail } from '@/api/equipment';
import { addbrowserecord } from '@/api/userInfo';
// import jxbSendcarDetail from '@/components/jxb-sendcar-detail';
...
export default {
	...
	async onLoad(options) {
		...
		//测试获取dict
		let dicts = await getDicts("report_xpq")
	},
	methods: {
		// 隐藏顶部导航
		hideNavDetail() {
      ...
      if (this.userInfo.promo_code) {
		...
		async getProductDetail() {
			await this.$http.get(`${productDetail}`, {params:{id: this.productId}}).then(async r => {
					this.loading = false;
