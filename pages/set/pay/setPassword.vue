		...
		<block v-if="styleLoginType === 'one'">
			<view class="container">
				...
				<view class="wrapper">
					...
					<view class="welcome"> {{ type == 1 ? '设置支付密码' : '找回密码' }}！ </view>
...
export default {
	...
	onLoad(options) {
		const time =
			dayjs().valueOf() / 1000 - uni.getStorageSync('pwdSmsCodeTime');
		if (time < 60) {
			this.codeSeconds =
		...
		async getSmsCode() {
			...
			await this.$http
				...
				.then(r => {
					...
					uni.setStorageSync('pwdSmsCodeTime', dayjs().valueOf() / 1000);
		...
		async toUpdatePassword() {
			...
			await this.$http
				...
				if(r==-1){
					...
					this.$mRouter.push({ route: '/pages/user/user' });
				})
				.catch(() => {
					this.btnLoading = false;
				});
