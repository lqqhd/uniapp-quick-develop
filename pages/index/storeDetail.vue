...
export default {
	...
	methods: {
		navToByStoreId(route) {
      		this.navTo(route+`?storeId=`+this.storeId);
      },
      // 跳至广告图指定页面
      		indexTopToDetailPage(route) {
      		   var url = route.url
...
// 推荐品牌
    		async getBrandList(storeId) {
    			await this.$http.get(`${homeBrand}`,{params:{storeId:storeId}}).then(r => {
    				this.brandList = r;
    			});
    		},
    		// 属性类别
		...
		// 轮播图列表 1 轮播图 2 新品推荐广告 3 人气推荐广告 4热门推荐广告
                  		async getBannerList(storeId) {
                  			await this.$http.get(`${bannerList}`,{params:{storeId:storeId}}).then(r => {
    				...
    				// 公告列表
              		async getannounceList(storeId) {
              			await this.$http.get(`${noticeList}`,{params:{storeId:storeId}}).then(r => {
              				this.announceList = r;
              			});
              		},
			// 热门商品列表
    		async getHotProductList(storeId) {
    			await this.$http.get(`${homeNewProduct}`,{params:{storeId:storeId}}).then(r => {
    				this.hotProductList = r;
    			});
    		},
    		// 推荐商品列表
            		async getRecommendProductList(storeId) {
            			await this.$http.get(`${homeRecommendProduct}`,{params:{storeId:storeId}}).then(r => {
            				this.recommendProductList = r;
            			});
            		},

                    		// 猜您喜欢商品列表
                            		async getGuessYouLikeProductList(storeId) {
                            			await this.$http.get(`${guessYouLikeProductList}`,{params:{storeId:storeId}}).then(r => {
                            				this.guessYouLikeProductList = r;
                            			});
                            		},
		// 跳转至商品详情页
