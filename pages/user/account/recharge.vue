...
export default {
	data() {
		return {
			...
			amountList: [{price:50,give_price:5},{price:100,give_price:10},{price:200,give_price:20}], // 预设3个可选快捷金额
			payType: 1, // 支付类型
			userInfo: {},
			loading: false,
			providerList: [],
		...
		async initData(options) {
			...
			await this.$http
				...
				.then(r => {
					...
					this.amountList = r;
					this.inputAmount = (r[0] && r[0].price) || 0.01;
					this.inputAmountGive = (r[0] && r[0].give_price) || 0;
					this.getPayTypeList();
				})
				.catch(() => {
					this.pageLoading = false;
