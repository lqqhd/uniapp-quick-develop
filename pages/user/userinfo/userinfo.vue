		...
		<view class="user-section" :class="'bg-' + themeColor.name">
			...
			<view class="portrait-box">
				<avatar
					...
					:avatarSrc="profileInfo.image || headImg"
...
export default {
	...
	methods: {
		...
		handleUploadFile(data) {
			...
			this.$http.upload(uploadImage, {filePath:filePath,name: 'file',formData:{type:'avatar'}}).then(res => {
				console.log('头像上传成功，res='+res);
					_this.profileInfo.image = res;
				}).catch(err => {
					console.log('头像上传失败 fail----，失败原因：'+err);
				});
		},
		// 监听日期更改
		bindDateChange(e) {
