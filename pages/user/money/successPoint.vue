<template>
	<view class="pay-success" v-if="!loading">
		...
		<view class="pay-card">
			<view class="pay-card-money">{{moneySymbol}}{{ orderDetail.price }}</view>
...
<script>
import { querypointmallorderdetail,ordersByCode  } from '@/api/userInfo';
...
export default {
	...
	methods: {
		...
		async getOrderDetail() {
			await this.$http
				.get(`${querypointmallorderdetail}`, {params:{
					id: this.id, // 订单id
