		...
		<view class="order-detail" v-if="orderDetail">
			<view class="rf-address-section">
				<view class="order-content">
					...
					<view class="cen">
						<view class="top">
							<uni-tag class="pickup-tag" v-if="orderDetail.orderAttr.pickUpAddress" type="error" text="自提点" size="small" />
							<text class="name">{{ orderDetail.orderAttr.receiptName || orderDetail.pickup }}</text>
							<text class="mobile">{{ orderDetail.orderAttr.receiptMobile || orderDetail.pickup }}</text>
						</view>
						<text class="address">{{ orderDetail.orderAttr.receiptAddress || orderDetail.pickup }} {{ orderDetail.orderAttr.receiptDetailAddress || orderDetail.pickup }}</text>
					</view>
			...
			<view class="rf-goods-section">
				<view class="g-header b-b">
					<view class="title">
						<image class="cover" :src="logo"></image>
						{{ orderDetail.storeName && orderDetail.storeName || appName }}
					</view>
				...
				<!-- 商品列表 -->
				<view class="g-item" v-for="(item, index) in orderDetail.orderSkus" :key="index" @tap="navTo(`/pages/product/product?id=${item.spuId}`)">
					...
					<view class="right">
						...
						<text class="spec">{{ item.skuName || singleSkuText }} * {{ item.num }}</text>
						<view class="price-box">
							<text class="price-wrapper">
								<text v-if="item.point_exchange_type == 2">
									实付<text :class="'text-' + themeColor.name">{{ moneySymbol }}{{ item.price }}</text>
									<text class="original-price" v-if="item.price > item.price">{{ moneySymbol }}{{ item.price }}</text>
								...
								<text v-else>
									<text :class="'text-' + themeColor.name">{{ moneySymbol }}{{ item.price }}</text>
									<text class="original-price">{{ moneySymbol }}{{ item.skuPrice }}</text>
...
<!--							<text class="status spec-color" @tap.stop="">{{item | filterProductStatus}}</text>-->
						...
						<view class="btn-box" v-if="item.gift_flag !== '1'">
							...
							<button class="action-btn" v-if="
									orderDetail.status == '2' &&
										orderDetail.canRefund
								"
							 @tap.stop="navToRefund(item, 1)">
								申请退款
							</button>
							<button class="action-btn" v-if="
									( orderDetail.status == 4) && orderDetail.canReturn
								"
							...
							<button class="action-btn" v-if="
									orderDetail.status == 4 &&
										(orderDetail.refund_status == 0 || orderDetail.refund_status == -3) && orderDetail.isVirtual !== '1'
								"
							...
							<button class="action-btn" v-if="
								...
								"
							 @tap.stop="
									handleCloseOrderRefundApply(orderDetail.status, item.id)
								">
								取消退款
							</button>
						</view>
				...
				<!-- 赠品列表 -->
        				<view class="g-item" v-for="(item1, index) in orderDetail.orderGifts" :key="index" >
        					<rf-image :preview="false" :src="item1.url"></rf-image>
        					<view class="right">
        						<text class="title clamp in2line">
        							<uni-tag class="pickup-tag"  type="error" text="赠品" size="small" />
        							{{ item1.skuName }}
        						</text>
        						<text class="spec"> * {{ item1.num }}</text>
        					</view>
        				</view>
			</view>
			...
			<view class="yt-list">
				<view class="yt-list-cell b-b">
					...
					<text class="cell-tip copy" :class="'text-' + themeColor.name" @tap.stop="copy(orderDetail.orderCode)">复制</text>
				</view>
			</view>
			<view class="yt-list">
				<view class="yt-list-cell b-b">
			...
			<view class="yt-list" v-if="this.orderDetail.orderType==1">
				<view class="yt-list-cell b-b">
					<text class="cell-tit clamp">阶段一
						<span v-if="this.orderDetail.presaleOnePayed">【已支付】</span>
			...
			<view class="yt-list" v-if="this.orderDetail.orderType==1">
				<view class="yt-list-cell b-b">
					<text class="cell-tit clamp">阶段二
						<span v-if="this.orderDetail.presaleStatus==2">【已支付】</span>
						<span v-else>【待支付】</span>
					</text>
					<text class="cell-tip">
...
<!--				</view>-->
			...
			<view class="yt-list">
				<view class="yt-list-cell b-b">
					...
					<text class="cell-tip" :class="'text-' + themeColor.name">{{ moneySymbol }}{{ (orderDetail.originalPrice || orderDetail.product_money) | keepTwoDecimal }}</text>
				</view>
				<!--				<view class="yt-list-cell b-b" v-if="orderDetail.concessionalRate > 0">-->
				<!--					<text class="cell-tit clamp">优惠金额</text>-->
				<!--					<text class="cell-tip red">-￥ {{ orderDetail.concessionalRate }}</text>-->
				<!--				</view>-->
				<!--营销金额优惠-->
				...
				<view class="yt-list-cell b-b">
					...
					<text class="cell-tip" :class="'text-' + themeColor.name">
						<text>{{ moneySymbol }}{{ orderDetail.freightPrice }}</text>
					</text>
				</view>
<!--				<view class="yt-list-cell b-b" v-if="orderDetail.orderAttr.invoiceCompanyName">-->
...
<!--				</view>-->
				<view class="yt-list-cell b-b" v-if="orderDetail.tax_money > 0">
					...
					<text class="cell-tip" :class="'text-' + themeColor.name">
						<text>{{ moneySymbol }}0</text>
					</text>
				</view>
				<view class="yt-list-cell b-b">
					<text class="cell-tit clamp">实付金额</text>
					<text class="cell-tip" :class="'text-' + themeColor.name">{{ moneySymbol }}{{ orderDetail.price }}</text>
				</view>
			</view>
...
<!--			</view>-->
			<!-- 底部 -->
			<view class="footer">
				<button class="action-btn" :class="'bg-' + themeColor.name" v-if="orderDetail.status == 5" @tap="handleOrderOperation(orderDetail, 'delete')">
					删除订单
				</button>
				<button class="action-btn" :class="'text-' + themeColor.name" v-if="orderDetail.status == 1" @tap="handleOrderOperation(orderDetail, 'close')">
					取消订单
				</button>
				<button class="action-btn" :class="'bg-' + themeColor.name" v-if="orderDetail.status == 1 &&orderDetail.presaleOrderCanPay"
				 @tap="navTo(`/pages/user/money/pay?money=${orderDetail.orderType==='1'?orderDetail.presalePrice:orderDetail.price}`+'&&orderCode='+orderDetail.orderCode)">
					立即支付
				</button>
<!--				<button class="action-btn" :class="'text-' + themeColor.name" v-if="-->
<!--						(orderDetail.status == 4 ||-->
<!--							orderDetail.status == 3 ) &&-->
<!--							orderDetail.orderType != 7-->
<!--					"-->
<!--				 @tap="handleOrderOperation(orderDetail, 'shipping')">-->
<!--					查看物流-->
<!--				</button>-->
<!--				<navigator url="/pages/set/invoice/invoice?source=1">-->
<!--					<button v-if="orderDetail.price > 0 && !orderDetail.invoice" class="action-btn" :class="'bg-' + themeColor.name">-->
<!--						申请开票-->
<!--					</button>-->
<!--				</navigator>-->
				<button class="action-btn" :class="'bg-' + themeColor.name" v-if="
						( orderDetail.status == 4) &&
							orderDetail.evaluationStatus == 0
					"
				 @tap="handleOrderOperation(orderDetail, 'evaluation')">
					评价
				</button>
				<button class="action-btn" :class="'bg-' + themeColor.name" v-if="orderDetail.status == 3" @tap="handleOrderOperation(orderDetail, 'delivery')">
					确认收货
				</button>
			</view>
		</view>
				...
				await this.$http
					...
					.then(r => {
						...
						if (this.invoiceItem.id && !this.orderDetail.invoice) {
							this.$http.get(configList, {params:{
