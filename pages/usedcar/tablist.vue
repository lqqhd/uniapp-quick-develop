...
<!--				<rf-pick-regions-all class="region" :style="{marginTop:inputTop+'px'}" :addressData="addressData" @getRegions="handleGetRegions"></rf-pick-regions-all>-->
				<uni-right-selecter :filterData="regionFilterData" isSimple :defaultSelected="regionDefaultSelected" title="区域" @confirm="regionConfirm"></uni-right-selecter>
...
<!--		</view>-->
		<view class="product-list-wrapper">
				<jxb-equipment-list :bottom="0" :list="productList" :isList="isList" :style="{paddingTop: dropScreenH+10 + 'upx' }"></jxb-equipment-list>
		</view>
		<rf-load-more
			:status="loadingType"
...
<script>
	...
	export default {
      ...
      ...mapMutations(['setCartNum']),
      	...
      	if(e.value[0][2] !== null){
					this.queryParams.countryId = e.value[0][2];
				}else{
      		delete this.queryParams.countryId
				}
			...
			confirm(e){
				...
				if(this.isNotNull(e.value[0][0])){
					this.queryParams.firstCateId = e.value[0][0];
				}else{
					delete this.queryParams.firstCateId;
				}
				if(this.isNotNull(e.value[0][1])){
					this.queryParams.secondCateId = e.value[0][1];
				}else{
					delete this.queryParams.secondCateId;
				}
				//价格，格式：20-30
				if(this.isNotNull(e.value[1][0][0])){
					this.queryParams.minPrice = e.value[1][0][0].split("-")[0];
					this.queryParams.maxPrice = e.value[1][0][0].split("-")[1];
				}else{
				...
				if(this.isNotNull(e.value[3][0][0])){
					this.queryParams.source = e.value[3][0][0];
				}else{
					delete this.queryParams.source;
				}
				//表显小时数
				if(e.value[3][1][0]){
					this.queryParams.minUsedHours = e.value[3][1][0].split("-")[0];
					this.queryParams.maxUsedHours = e.value[3][1][0].split("-")[1];
				}else{
					this.queryParams.minUsedHours = 0;
					this.queryParams.maxUsedHours = 999999;
				}
				//发布时间
				if(this.isNotNull(e.value[3][2][0])){
					this.queryParams.publishDays = e.value[3][2][0];
				}else{
