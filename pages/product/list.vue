			...
			<view class="rf-drawer-box" :style="{paddingTop:height+'px'}">
				<scroll-view class="rf-drawer-scroll" scroll-y :style="{height:drawerH+'px'}">
					...
					<view class="rf-drawer-content rf-flex-attr">
						<view class="rf-attr-item" :class="[item.isActive ? `bg-${themeColor.name} rf-btmItem-active` : 'rf-btmItem-normal']" v-for="(item, index) in productCateList"
						 :key="item.id" @tap.stop="cateBtnSelected(index)">
							<view class="rf-attr-ellipsis">{{ item.name }}</view>
						</view>
					</view>
					...
					<view class="rf-drawer-content rf-flex-attr">
						<view class="rf-attr-item" :class="[item.isActive ? `bg-${themeColor.name} rf-btmItem-active` : 'rf-btmItem-normal']" v-for="(item, index) in brandList"
						:key="item.id" @tap.stop="brandBtnSelected(index)">
							<view class="rf-attr-ellipsis">{{ item.name }}</view>
						</view>
					</view>
...
<script>
	...
	export default {
      ...
      ...mapMutations(['setCartNum']),
			...
			initData(options) {
			  let params = {};
			  if (options.secondCateId) {
					params.secondCateId = options.secondCateId;
			  }
			  if (options.firstCateId) {
        					params.firstCateId = options.firstCateId;
        			  }
        			  if (options.thirdCateId) {
                					params.thirdCateId = options.thirdCateId;
                			  }
                			  if (options.storeId) {
