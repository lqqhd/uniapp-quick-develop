<template>
	<view class="user">
		...
		>
			...
			<!--推广中心-->
			<view class="promotion-center" v-show="false">
				<list-cell
					icon="iconqianbao"
					:iconColor="themeColor.color"
					@eventClick="navTo('/pages/user/fenxiao/fenxiao')"
					title="推广中心"
				></list-cell>
				<view class="tj-sction">
					<view
						class="tj-item"
						@tap="navTo('/pages/user/fenxiao/list')"
					>
						<text class="num" :class="orderStatics.commissonMoney >= 0 ? 'text-'+themeColor.name : ''">
							{{ orderStatics.commissonMoney || '0' }}
						</text>
						<text>累加佣金</text>
					</view>
					<view
						class="tj-item"
						@tap="navTo('/pages/user/fenxiao/fenxiao')"
					>
						<text class="num" :class="userInfo.commission > 0 ? 'text-'+themeColor.name : ''">
					...
					<view
						class="tj-item"
						@tap="navTo('/pages/user/fenxiao/withDraw?state=-1')"
					>
						<text class="num" :class="orderStatics.withdrawMoney >= 0 ? 'text-'+themeColor.name : ''">
							{{ orderStatics.withdrawMoney || '0' }}
						</text>
						<text>已提现佣金</text>
					</view>
				</view>
			...
			<!-- 浏览历史 -->
			<view class="history-section" v-show="false">
				<list-cell
					icon="iconzuji"
					:iconColor="themeColor.color"
				...
				<view v-if="hasLogin">
					<scroll-view scroll-x class="h-list-history" v-if="footPrintList.length > 0">
						<view class="h-item-history" v-for="item in footPrintList" :key="item.id"
									@tap.stop="navToProduct(item.marketing_type, item.sku.spuId)">
							<image
								:src="item.sku.url"
								class="h-item-img"
								:preview="false"
							...
							<view class="h-item-text">
								<text class="in2line">{{ item.sku.name }}</text>
							</view>
						</view>
					</scroll-view>
					<view
...
export default {
	...
	methods: {
		...
		setSectionData(data) {
			...
			this.amountList[2].value = data.customerPoint || 0;
			// 更新userInfo缓存
