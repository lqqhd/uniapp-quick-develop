				...
				>
					...
					>
						<view
							...
							@tap.stop="
								navTo(`/pages/product/product?id=${goodsItem.spuId}`)
							"
...
export default {
	...
	methods: {
		...
		async initData() {
			...
			async getNavList() {
			await this.$http
              				.get(`${querypresalepic}`, {params:{storeId:this.storeId}})
              				.then(async r => {
              						this.prePic=r;
              					this.loading = false;
              				})
					...
					await this.$http
        				.get(`${querypresalecates}`, {params:{storeId:this.storeId}})
        				.then(async r => {
        						this.navList=r;
...
console.log(this.orderList)
		...
		async getGuessYouLikeList() {
			await this.$http.get(`${guessYouLikeList}`,{params:{num:6}}).then(r => {
