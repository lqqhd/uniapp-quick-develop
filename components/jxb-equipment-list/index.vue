<template>
	<view class="rf-product-list">
		<view class="rf-product-list-container">
			<block v-for="(item, index) in list" :key="index">
				...
				>
					...
					<view class="rf-equipment-content">
						<view class="rf-pro-tit">{{ item.name }}</view>
						<view v-if="isSendcar" class="rf-factory-subtitle">{{ item.factoryDate }}年 | {{ item.usedHours }}小时</view>
						<view v-if="!isSendcar" class="rf-factory-subtitle">{{ item.factoryDate }}年 | {{ item.usedHours }}小时 | {{item.provinceName}}-{{item.cityName}}</view>
						<view v-if="item">
							<view class="rf-pro-price">
								<text class="rf-sale-price" style="color: white;">_</text>
							</view>
							<view class="rf-pro-pay">
								<text class="rf-sale-price" style="color: red;font-weight: bold">{{ item.price }}万</text>
								<text @tap.stop="showPhoneDialog(item)" class="btn" >免费询价</text>
							</view>
						</view>
					</view>
				</view>
		...
		<!-- 提交信息 -->
		<!-- 提交信息 -->
		<tui-modal :show="showPhoneModal" @click="dialogConfirm" :button="modalButton" title="提示" :content="phone !== null && phone !== '' && phone !== undefined?'拨打'+phone+'咨询购买本设备!':'卖家未留联系方式'"></tui-modal>
		<!-- 规格-模态层弹窗 -->
...
export default {
    ...
    ...mapMutations(['setCartNum']),
		...
		navToDetail(id) {
			if(this.isSendcar){
				let route = `/pages/sendcar/product?id=${id}`
				this.$mRouter.push({route});
			}else{
				let route = `/pages/usedcar_sub/product?id=${id}`
				if(this.isEdit){
					route = `/pages/usedcar_sub/update?id=${id}`
				}
				this.$mRouter.push({route});
			}
