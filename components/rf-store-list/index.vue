		...
		<view class="rf-product-list-container">
			<block v-for="(item, index) in list" :key="index">
				...
				<view
					...
					@tap.stop="navTo(`/pages/index/storeDetail?id=${item.id}`)"
					:class="[isList ? 'rf-product-flex-list' : '']"
					hover-class="hover"
					:hover-start-time="150"
				>
					<view class="rf-product-image-wrapper">
						<image :src="item.avatarPicture" mode="widthFix" :preview="false" :class="[isList?'rf-product-list-img':'rf-product-img']" ></image>
						<text class="sketch in1line">{{ item.companyName }}</text>
					...
					<view class="rf-pro-content">
						...
						<view v-if="item">
							<view class="rf-pro-price">
								<text class="rf-sale-price" :class="'text-'+themeColor.name">关注 {{ item.followNum }}</text>
									<text class="rf-sale-price" :class="'text-'+themeColor.name"></text>
								<text class="rf-sale-price" > 评分{{ item.aveScore }}</text>
							</view>
		...
		<view class="rf-product-list-container" v-if="!isList">
			<block v-for="(item, index) in list" :key="index">
				...
				>
					...
					<view class="rf-pro-content">
						<view class="rf-pro-tit">{{ item.storeName }}</view>
						<view v-if="item">
							<view class="rf-pro-price">
								<text class="rf-sale-price" :class="'text-'+themeColor.name">{{ item.followNum }}</text>
								<text class="rf-factory-price" >{{ moneySymbol }}{{ item.aveScore }}</text>
							</view>
							<view class="rf-pro-pay">
								<text
									><text :class="'text-'+themeColor.name">{{
