<template>
	<view>
		<view>
				<view v-if="!isSimple" class="first-menu" @tap="togglePage(0)">
					<text :class="shadowActiveMenuArr.length === 0 ||shadowActiveMenuArr[0] === null || shadowActiveMenuArr[0][0] === null?'cell-tip-grey':'cell-tip'">{{menu.length>0?menu[0].name:""}}</text>
					<uni-icons color="#aaa" type="arrowright" size="15" class="cell-more"/>
				</view>
			<view v-if="isSimple"  @tap="togglePage(0)" :style="{marginTop:navbarHeight +'px',marginLeft:'10px'}">
				<text>{{menu[0] === undefined?"":menu[0].name}}</text>
				<uni-icons color="#aaa" type="arrowdown" size="15" class="cell-more"/>
			</view>
			...
			<block v-for="(page,page_index) in subData" :key="page_index">
				<view class="sub-menu-class" :class="{'show':showPage===page_index,'hide':pageState[page_index]!==true}">
					<view style="width: 100%;display: block">
						...
						<view class="block-content" :style="{height:statusBarHeight +'px'}">
...
import uniIcons from "../uni-icons/uni-icons.vue";
	export default {
		...
		computed:{
			...
			statusBarHeight(){
				var that = this;
				return uni.getSystemInfoSync().screenHeight - uni.getSystemInfoSync().statusBarHeight - 44
			},
			navbarHeight(){
				var that = this;
		...
		methods: {
			getTitleHeight() {
				...
				uni.getSystemInfo({
					success: res => {
						this.statusBarHeight2 = res.statusBarHeight
						if (res.system.indexOf('Android') !== -1) {
							this.titleHeight = 48 + this.statusBarHeight2
							this.isIOS = false
						} else if (res.system.indexOf('iOS') !== -1) {
							this.titleHeight = 44 + this.statusBarHeight2
							this.isIOS = true
						}
					}
				})
			...
			setMenuName(){
				for(var i=0;i<this.activeMenuArr.length;i++){
					...
					if(this.subData[i] && this.subData[i].type === 'hierarchy'){
				...
				if (level3_index != null || level2_index != null || (level1_index != null && this.subData[page_index].submenu[level1_index].submenu.length == 0)) {
					...
					if(this.updateMenuName){
						if(this.isSimple){
							...
							this.menu[page_index].name = this.subData[page_index].submenu[level1_index].name + (level2_index != null && sub.name?"-"+sub.name:"") + (level3_index != null && sub.submenu[level3_index].name?"-"+sub.submenu[level3_index].name:"") ;
						}
					}
					...
					if (typeof(item[0]) == 'object') {
						...
						if(value[i].length>=2  && item[1]!=null){
							if(submenu.submenu.length>0){
								...
								name[i][1] = submenu.hasOwnProperty('name')?submenu.name:null;
							}else{
								value[i][1] = null
								name[i][1] = null
							}
							if(value[i].length>=3 && item[2]!=null){
								if(submenu.submenu.length>0){
									...
									name[i][2] = submenu.hasOwnProperty('name')?submenu.name:null;
								}else{
									value[i][2] = null;
									name[i][2] = null;
								}
