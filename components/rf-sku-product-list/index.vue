		...
		<view class="rf-product-list-container">
			<block v-for="(item, index) in list" :key="index">
				...
				<view
					...
					@tap.stop="navTo(`/pages/product/product?id=${item.sku.spuId}`)"
					:class="[isList ? 'rf-product-flex-list' : '']"
					hover-class="hover"
					:hover-start-time="150"
				>
					<view class="rf-product-image-wrapper">
						<image :src="item.sku.url" mode="widthFix" :preview="false" :class="[isList?'rf-product-list-img':'rf-product-img']" ></image>
					...
					<view class="rf-pro-content">
						<view class="rf-pro-tit">{{ item.sku.name }}</view>
						<view v-if="item">
							<view class="rf-pro-price">
								<text class="rf-sale-price" :class="'text-'+themeColor.name">{{ moneySymbol }}{{ item.sku.price }}</text>
								<text class="rf-factory-price" v-if="item.sku.price > item.sku.price">{{ moneySymbol }}{{ item.sku.price }}</text>
							</view>
							<view class="rf-pro-pay">
								<text
									>剩余<text :class="'text-'+themeColor.name">{{
										item.sku.stock | filterTotalSales
									}}</text>
									</text
								>
								<text
